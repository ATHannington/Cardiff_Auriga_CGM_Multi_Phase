...
! or, for debugging:    gfortran -o2 -g -fbounds-check -o run RadTrans_MainCode.F90
!
!To RUN:
!   ./run
...
IF (TElist==1) THEN                                      ! [IF] printout sanctioned, [THEN]
    print*," "
    PRINT*,"TElist .eq. 1! Temperature Diagnostics: "
    PRINT*,"TEk,teT(TEk)"
    DO TEk=0,TEkTOT,10
        WRITE (6,"(I5,F11.3)") TEk,teT(TEk)
    ENDDO
ENDIF                                                    ! [ENDIF]
