...
for T in TRACERSPARAMS["targetTLst"]:
    ...
    for analysisParam in saveParams:
        ...
        for snap in range(
            ...
            data, _, _ = GetIndividualCellFromTracer(
                Tracers=dataDict[key]["trid"][whereGas],
                Parents=dataDict[key]["prid"][whereGas],
...
for temp in Tlst:
    ...
    startSubHaloIDDataFull, _, _ = GetIndividualCellFromTracer(
        Tracers=dataDict[startkey]["trid"],
        Parents=dataDict[startkey]["prid"],
        CellIDs=dataDict[startkey]["id"],
        SelectedTracers=dataDict[startkey]["trid"],
        Data=dataDict[startkey]["SubHaloID"],
    )
    endSubHaloIDDataFull, _, _ = GetIndividualCellFromTracer(
        Tracers=dataDict[endkey]["trid"],
        Parents=dataDict[endkey]["prid"],
        CellIDs=dataDict[endkey]["id"],
        SelectedTracers=dataDict[endkey]["trid"],
    ...
    haloFracStart = float(
        np.shape(np.where(startSubHaloIDDataFull == int(TRACERSPARAMS["haloID"]))[0])[0]
    ) / float(startNtracers)
    haloFracEnd = float(
        np.shape(np.where(endSubHaloIDDataFull == int(TRACERSPARAMS["haloID"]))[0])[0]
    ) / float(endNtracers)
    ...
    for ii in range(len(Tlst)):
        ...
        for dataset in violinData:
            q1, med, q3 = np.percentile(
                dataset,
                [
                    int(TRACERSPARAMS["percentileLO"]),
                    50,
                    int(TRACERSPARAMS["percentileUP"]),
                ],
                axis=0,
            )
            quartile1.append(q1)
        ...
        currentAx.vlines(
            plotXdata, quartile1, quartile3, color="k", linestyle="-", lw=3
        )
        currentAx.vlines(
            plotXdata, whiskers_min, whiskers_max, color="k", linestyle="-", lw=1
        )
        ...
        currentAx.set_ylim(
            ymin=min(datamin, np.nanmin(whiskers_min)),
            ymax=max(datamax, np.nanmax(whiskers_max)),
        )
