...
for targetT in TRACERSPARAMS["targetTLst"]:
    ...
    GasSelect = np.where(
        (snapGas.data["T"][whereGas] >= 1.0 * 10 ** (targetT - TRACERSPARAMS["deltaT"]))
        & (
    ...
    snapRangeMin = [
        zz
        for zz in range(
            int(TRACERSPARAMS["snapnum"]), (int(TRACERSPARAMS["snapMin"]) - 1), -1
        )
    ]
    snapRangeMax = [
        zz
        for zz in range(
            int(TRACERSPARAMS["snapnum"]),
            min(
                int(TRACERSPARAMS["snapnumMAX"]) + 1, int(TRACERSPARAMS["snapMax"]) + 1
            ),
            1,
        )
    ]
    ...
    for snapSet in snapRange:
        ...
        for snap in snapSet:
            ...
            TracersCFT, CellsCFT, CellIDsCFT, ParentsCFT = GetCellsFromTracers(
                snapGas, snapTracers, TracersOld
            )
            ...
            CellsCFT["Lookback"] = np.array(
                [lookback for jj in range(0, len(CellsCFT["T"]))]
            )

            # Save snap number
            CellsCFT["Snap"] = np.array(
                [int(snap) for jj in range(0, len(CellsCFT["T"]))]
            )
        ...
        if Tkey == f"T{int(targetT)}":
            for k, v in value.items():
                if (k != "Lookback") & (k in saveParams):
                    ...
                    if (
                        (f"{k}median" not in plotData.keys())
                        or (f"{k}UP" not in plotData.keys())
                        or (f"{k}LO" not in plotData.keys())
                    ):
                        plotData.update(
                            {
                                f"{k}median": weightedperc(
                                    data=v[whereGas],
                                    weights=FullDict[key]["mass"][whereGas],
                                    perc=50.0,
                                )
                            }
                        )
                        plotData.update(
                            {
                                f"{k}UP": weightedperc(
                                    data=v[whereGas],
                                    weights=FullDict[key]["mass"][whereGas],
                                    perc=TRACERSPARAMS["percentileUP"],
                                )
                            }
                        )
                        plotData.update(
                            {
                                f"{k}LO": weightedperc(
                                    data=v[whereGas],
                                    weights=FullDict[key]["mass"][whereGas],
                                    perc=TRACERSPARAMS["percentileLO"],
                                )
                            }
                        )
                    else:
                        plotData[f"{k}median"] = np.append(
                            plotData[f"{k}median"],
                            weightedperc(
                                data=v[whereGas],
                                weights=FullDict[key]["mass"][whereGas],
                    ...
                    if f"Lookback" not in plotData.keys():
                        ...
                        plotData[f"{k}"] = np.append(plotData[f"{k}"], np.median(v))
                else:
                    #
                    #   !!! NOT TESTED  !!!
                    #
