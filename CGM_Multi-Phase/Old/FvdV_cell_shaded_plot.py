...
rhocrit = (
    3.0
    * (s.omega0 * (1 + s.redshift) ** 3.0 + s.omegalambda)
    * (s.hubbleparam * 100 * 1e5 / (c.parsec * 1e6)) ** 2.0
    / (8.0 * pi * c.G)
)
rhomean = (
    3.0
    * (s.omega0 * (1 + s.redshift) ** 3.0)
    * (s.hubbleparam * 100 * 1e5 / (c.parsec * 1e6)) ** 2.0
    / (8.0 * pi * c.G)
)
...
meanweight = sum(s.gmet[:, 0:9], axis=1) / (
    sum(s.gmet[:, 0:9] / elements_mass[0:9], axis=1) + s.ne * s.gmet[:, 0]
)
Tfac = (
    1.0
    / meanweight
...
gasdens = s.rho / (c.parsec * 1e6) ** 3.0 * c.msol * 1e10
gasX = s.gmet[:, 0]
...
pcm3 = axes[1, 0].pcolormesh(
    ...
    norm=matplotlib.colors.LogNorm(vmin=1e4, vmax=1e7),
    cmap=cmap,
    rasterized=True,
)
