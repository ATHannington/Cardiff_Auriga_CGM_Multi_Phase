...
for snap in TRACERSPARAMS['phasesSnaps']:
    ...
    lazyLoadBool,SUBSET=None,snapNumber=snap,saveTracers=False,TFCbool=False,loadonlyhalo=False)
...
for (rin,rout) in zip(TRACERSPARAMS['Rinner'],TRACERSPARAMS['Router']):
    ...
    for snap in TRACERSPARAMS['phasesSnaps']:
        ...
        for weightKey in weightKeys:
            ...
            selectTime = abs(FullDict[(f"T{float(Tlst[0])}",f"{rin}R{rout}", f"{int(TRACERSPARAMS['selectSnap'])}")]['Lookback'][0] - ageUniverse)
            currentTime = abs(FullDict[(f"T{float(Tlst[0])}",f"{rin}R{rout}", f"{int(snap)}")]['Lookback'][0] - ageUniverse)
    ...
    #- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
            ...
            xdataTracers = np.log10(TracersFinalDict[tkey]['rho_rhomean'][whereTracersGas])
            ydataTracers = np.log10(TracersFinalDict[tkey]['T'][whereTracersGas])
            massTracers = TracersFinalDict[tkey]['mass'][whereTracersGas]
            weightDataTracers = TracersFinalDict[tkey][weightKey][whereTracersGas] * massTracers
            ...
            img2 = ax[1].pcolormesh(xtracers, ytracers, finalHistTracers, cmap=colourmap,vmin=zmin,vmax=zmax, rasterized=True)
    ...
    #- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
            ...
            opslaan =  "./" + saveHalo + "/" + f"{int(rin)}R{int(rout)}" + "/"+ f"Tracers_selectSnap{int(TRACERSPARAMS['selectSnap'])}_snap{int(snap)}_{weightKey}_PhaseDiagram.pdf"
            ...
            opslaan =  "./" + saveHalo + "/" + f"{int(rin)}R{int(rout)}" + "/"+ f"Tracers_selectSnap{int(TRACERSPARAMS['selectSnap'])}_snap{int(snap)}_{weightKey}_PhaseDiagram_Individual-Temps.pdf"
            plt.savefig(opslaan, dpi = DPI, transparent = False)
            print(opslaan)
