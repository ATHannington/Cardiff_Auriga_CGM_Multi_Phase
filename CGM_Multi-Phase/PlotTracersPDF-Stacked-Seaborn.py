...
for dataKey in saveParams:
    ...
    for (rin,rout) in zip(TRACERSPARAMS['Rinner'],TRACERSPARAMS['Router']):
        ...
        for ii in range(len(Tlst)):
            ...
            selectKey = (f"T{T}",f"{rin}R{rout}",f"{int(TRACERSPARAMS['selectSnap'])}")
            ...
            opslaan = "./" + saveHalo + "/" + f"{int(rin)}R{int(rout)}" + "/"+ f"Tracers_selectSnap{int(TRACERSPARAMS['selectSnap'])}_snap{int(snap)}_T{T}_{dataKey}_PDF.pdf"
