def _inner_analysis(dataDict, T, ClusterID, FullKey, NtracersFull):
    ...
    gas.append(100.0 * (Ntracers / NtracersAll))

    halo0.append(
        100.0
        * (
            np.shape(np.where(SubHalo[whereGas] == int(TRACERSPARAMS["haloID"]))[0])[0]
            / Ntracers
        )
    )

    unbound.append(
        100.0 * (np.shape(np.where(SubHalo[whereGas] == -1)[0])[0] / Ntracers)
    )
    ...
    stars.append(
        ...
        * (
            np.shape(
                np.where(
                    (dataDict[FullKey]["age"] >= 0) & (dataDict[FullKey]["type"] == 4)
                )[0]
            )[0]
            / NtracersAll
        )
    ...
    ism.append(
        ...
        * (
            np.shape(
                np.where(
                    (dataDict[FullKey]["sfr"] > 0) & (dataDict[FullKey]["R"] <= 25.0)
                )[0]
            )[0]
            / NtracersAll
        )
    ...
    inflow.append(
        100.0
        * (
            np.shape(np.where(dataDict[FullKey]["vrad"][whereGas] < 0.0)[0])[0]
            / Ntracers
        )
    )
    outflow.append(
...
def clusteredData_analyse_cluster(
    dataDict, dtwparamStatsDict, Tlst, snapsRange, FullDict, ignoreParams
):
    ...
    for T in Tlst:
        ...
        for clusterID in clusters:
            ...
            outflow = []
            for ii, snap in enumerate(snapsRange):
                whereCluster = np.where(dataDict[Tkey]["clusters"][ii, :] == clusterID)[
                    0
                ]
                tmpsnapdataDict = {}
                for k, v in dataDict[Tkey].items():
                    if np.shape(np.shape(v))[0] > 1:
                        ...
                        tmpsnapdataDict.update({k: v})
                tmpsnapdataDict.update(
                    {"Ntracers": [np.shape(tmpsnapdataDict["type"])[0]]}
                )
...
for T in Tlst:
    ...
    for ii, snap in enumerate(snapsRange):
        FullKey = (f"T{T}", f"{int(snap)}")
        _, tridIndices, dtwIndices = np.intersect1d(
            FlatDataDict[key]["trid"][ii, :],
            dtwDict[key]["trid"][:, ii],
            return_indices=True,
        )
        tmp = {}
...
for T in Tlst:
    ...
    for ii, snap in enumerate(snapsRange):
        FullKey = (f"T{T}", f"{int(snap)}")
        _, tridIndices, _ = np.intersect1d(
            dataDict[FullKey]["trid"], dtwDict[key]["trid"][:, ii], return_indices=True
        )
        prids = dataDict[FullKey]["prid"][tridIndices]
        _, dataIndices, _ = np.intersect1d(
            prids, dataDict[FullKey]["id"], return_indices=True
        )
        data = {}
        for k, v in dataDict[FullKey].items():
            if k in saveParams + ["mass"]:
                data.update({k: v[dataIndices]})
        statsData = save_statistics(
            data,
            T,
            snap,
            TRACERSPARAMS,
            saveParams,
            DataSavepath=DataSavepath + "_DTW-Clusters",
...
StatsDF = fullData_analyse_cluster(
    FlatDataDict, paramStatsDict, Tlst, snapsRange, ClusterID=-1
)
clusterStatsDF = clusteredData_analyse_cluster(
    clusteredDict,
    dtwparamStatsDict,
    Tlst,
    snapsRange,
    FullDict=FlatDataDict,
    ignoreParams=ignoreParams,
)

finalStatsDF = pd.concat(
    [StatsDF, clusterStatsDF], axis=0, join="outer", sort=False, ignore_index=True
)
