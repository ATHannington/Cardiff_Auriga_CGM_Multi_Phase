...
for ii in range(len(Tlst)):
    ...
    ax.fill_between(plotData['Lookback'],plotData['n_HUP'],plotData['n_HLO'],\
    facecolor=colour,alpha=opacity,interpolate=True)
    ax.plot(plotData['Lookback'],plotData['n_Hmedian'],label=r"$T = 10^{%3.0f} K$"%(float(temp)), color = colour)
    ax.axvline(x=plotData['Lookback'][int(TRACERSPARAMS['snapnum']-TRACERSPARAMS['snapMin'])], c='red')

    ax.set_yscale('log')
    ax.set_xlabel(r"Lookback Time [$Gyrs$]")
    ax.set_ylabel(r"$n_H$ [c$m^{-3}$]")
...
opslaan = f"Tracers{int(TRACERSPARAMS['snapnum'])}n_H.png"
