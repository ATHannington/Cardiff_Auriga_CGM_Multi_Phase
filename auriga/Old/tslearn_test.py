...
for n_n in np.arange(n_n_min,n_n_max+n_n_delta,n_n_delta):
    ...
    start = time.time()
    km = TimeSeriesKMeans(n_clusters=2,n_init=n_init,metric="dtw",n_jobs=n_jobs).fit_predict(X_scaled)
    end = time.time()
    ...
    percent = (float(n_n)/float(n_n_max))*100.
...
plt.plot(plot_x_dat,fit(plot_x_dat), label=f"Polynomial Fit {coeffs[0]:0.04f}x^2 + {coeffs[1]:0.04f}x + {coeffs[2]:0.04f}", color="red", linestyle="-.")
plt.ylim(np.min(plot_y_dat), np.max(plot_y_dat))
plt.legend()
plt.xlabel("N Time Series")
plt.ylabel("Run Time [s]")
plt.title("DTW Scaling test - tslearn Package"  + \
