    ...
    for analysisParam in saveParams:
        ...
        if (dtwJoint == False):
            if (analysisParam in logParams):
                loadPath = DataSavepath + f"_T{T}_log10{analysisParam}_DTW-clusters"+DataSavepathSuffix
                print("\n" + f"[@T{T} log10{analysisParam}]: Saving Clusters data as: "+ loadPath)
            else:
                loadPath = DataSavepath + f"_T{T}_{analysisParam}_DTW-clusters"+DataSavepathSuffix
...
for analysisParam in saveParams:
    ...
    for ii in range(len(Tlst)):
        ...
        currentAx = fig.add_subplot(gs[1, int((nClusters/2)*ii):int((nClusters/2)*(ii+1))])

        currentAx.fill_between(tage,plotData[UP],plotData[LO],\
        facecolor=colour,alpha=opacityPercentiles,interpolate=False)
        currentAx.plot(plotXdata,plotData[median], color = colour, lineStyle=lineStyleMedian, linewidth = lineStyleMedian)
    ...
    if (dtwJoint == True):
        opslaan = f"Tracers_selectSnap{int(TRACERSPARAMS['selectSnap'])}_"+analysisParam+f"_IndividualsMedians-DTW-Joint-Clusters.pdf"
    else:
        opslaan = f"Tracers_selectSnap{int(TRACERSPARAMS['selectSnap'])}_"+analysisParam+f"_IndividualsMedians-DTW-Clusters.pdf"
