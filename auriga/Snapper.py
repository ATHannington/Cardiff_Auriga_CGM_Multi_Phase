...
def PlotHistogram(snap, snapnum, Nbins=500,Axes=[0,1],Range = [[-50,50],[-50,50]],WeightsLabel='mass',Normed=False):
    """
...
def PlotProjections(snapGas,snapnum,snapDM=None,snapStars=None,Axes=[0,1],zAxis=[2],boxsize = 400.,boxlos = 50.,pixres = 1.,pixreslos = 0.1, Nbins=500, DPI = 500,CMAP='inferno'):
    ...
    proj_dens = snapGas.get_Aslice("dens", box = [boxsize,boxsize],\
     center = imgcent, nx = int(boxsize/pixres), ny = int(boxsize/pixres),\
     nz = int(boxlos/pixreslos), boxz = boxlos, axes = Axes, proj = True, numthreads=16)
...
#               CURRENTLY ONLY DEFAULTS SUPPORTED
#------------------------------------------------------------------------------#
def _projection_movie(snapnum,SimDirectory,Axes,zAxis,boxsize,boxlos,pixres,pixreslos,Nbins,DPI,CMAP):
...
def ProjectionMovieLoop(SimDirectory,Start,End,n_processes,Axes,zAxis,boxsize,boxlos,pixres,pixreslos,Nbins,DPI,CMAP):
    """
    Function for Histogram 2D plotting of a loop (Start to Nsnaps) snaps.
    Args:
        SimDirectory        : REQUIRED! Directory path for simulation
    ...
    args_default = [SimDirectory,Axes,zAxis,boxsize,boxlos,pixres,pixreslos,Nbins,DPI,CMAP]
