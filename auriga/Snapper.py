...
#------------------------------------------------------------------------------#
    ...
    def PlotProjections(self,Snap,SnapNum,SnapDM=None,SnapStars=None,Axes=[0,1],boxsize = 400.,boxlos = 50.,pixres = 1.,pixreslos = 0.1, Nbins=500, DPI = 500,CMAP='inferno'):
        ...
        slice_nH    = Snap.get_Aslice("n_H", box = [boxsize,boxsize],\
         ...
         axes = Axes, proj = False, numthreads=1)
        ...
        #-----------#
        # print("pcm2")
        pcm2 = axes[0,1].pcolormesh(slice_nH['x'], slice_nH['y'], np.transpose(slice_nH['grid']),\
        norm = matplotlib.colors.LogNorm(vmin=1e-6,vmax=1e-1), cmap = cmap, rasterized = True)
        axes[0,1].set_title(r'Slice $n_H$ centred at z=0 kpc')
        ...
        #-----------#
        # print("pcm4")
        pcm4 = axes[1,1].pcolormesh(slice_B['x'], slice_B['y'], np.transpose(slice_B['grid']),\
        norm = matplotlib.colors.LogNorm(),\
        cmap = cmap, rasterized = True)
        # print("foo!")
        axes[1,1].set_title(r'Slice $B$ centred at z=0 kpc')
...
#               CURRENTLY ONLY DEFAULTS SUPPORTED
#------------------------------------------------------------------------------#
    def _projection_movie(self,snap,SimDirectory):
        snapper = Snapper()
        print(f"Starting {snap}")
        # load in the subfind group files
        snap_subfind = load_subfind(snap,dir=SimDirectory)

        # load in the gas particles mass and position. 0 is gas, 1 is DM, 4 is stars, 5 is BHs
        snapGas   = gadget_readsnap(snap, SimDirectory,loadonlytype = [0],lazy_load=True)
        snapDM    = gadget_readsnap(snap, SimDirectory,loadonlytype = [1],lazy_load=True)
        snapStars = gadget_readsnap(snap, SimDirectory,loadonlytype = [4],lazy_load=True)
        ...
        snapper.PlotProjections(Snap=snapGas,SnapNum=snap,SnapDM=snapDM,SnapStars=snapStars,DPI=200)

        return

    def ProjectionMovieLoop(self,SimDirectory,Start,End,n_processes):
        """
        Function for Histogram 2D plotting of a loop (Start to NSnaps) Snaps.
        Args:
            SimDirectory        : REQUIRED! Directory path for simulation
        ...
        args_default = [self.SimDirectory]
        ...
        [pool.apply_async(snapper._projection_movie,args=args) for args in args_list]
...
# Histogram 2D Movie:
Snapper4 = Snapper()
Snapper4.ProjectionMovieLoop(SimDirectory=simfile,Start=snap_start,End=snap_end,n_processes=n_processes)
